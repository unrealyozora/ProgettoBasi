...
#Tv Series Provider, with dataset from https://www.kaggle.com/datasets/sudhanshuy17/popular-tv-shows-data-set
class SeriesProvider(BaseProvider):
    ...
    #Pick a random series from the dataset
    def new_series(self):
...
fake_series = Faker()
fake_series.add_provider(SeriesProvider)
NUM_UTENTI = 1000
NUM_SERIE = 200
NUM_REGISTI = 50
NUM_ATTORI = 100
NUM_SOTTOSCRIZIONI = 2000
NUM_VISUALIZZAZIONI = 10000
...
with open('src/populate.sql', "w", encoding="utf-8") as f:
   ...
   for i in range(NUM_SERIE):
     fake_series.new_series()
     titolo=fake_series.series_name().replace("'", "''")
     if titolo in serie:
       continue
     anno_inizio=random.randint(1980, 2024)
     serie[titolo] = anno_inizio
     genere=random.choice(GENERI)
     descrizione=fake_series.series_description().replace("'", "''")
